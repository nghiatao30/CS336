...
def init():
    ...
    with open('app/data/cafe_index.json', 'r') as f:
        data_cafe = json.load(f)
...
def search(img, k=10, dataset='oxbuild'):
    ...
    if dataset == 'oxbuild':
        ...
        distance = [spatial.distance.cosine(i, query_vector) for i in data_cafe['vectors']]
        ids = np.argsort(distance)[:k]
        results = [data_cafe['paths'][id] for id in ids]
        # Apply re-ranking
    return results